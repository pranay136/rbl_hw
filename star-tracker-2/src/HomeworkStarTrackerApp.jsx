...
const defaultForm = {
  ...
  noComplaint: false,
  selfPlayHour: false,
  constructivePlayHour: false,
  bathAndBrush: false,
  goodBehaviorAtHome: false,
  noMessAllDay: false,
  listenedToParents: false,
  notes: "",
};
...
function calculateStars(entry) {
  ...
  if (entry.goodBehavior) {
    bonusStars += 1;
    bonusReasons.push("good behavior bonus");
  }
  if (entry.noComplaint) {
    bonusStars += 1;
    bonusReasons.push("no complaint bonus");
  }
  if (entry.selfPlayHour) {
    bonusStars += 1;
    bonusReasons.push("1 hour of self-play");
  }
  if (entry.constructivePlayHour) {
    bonusStars += 1;
    bonusReasons.push("1 hour of constructive play");
  }
  if (entry.bathAndBrush) {
    bonusStars += 1;
    bonusReasons.push("bath and brushing nicely");
  }
  if (entry.goodBehaviorAtHome) {
    bonusStars += 1;
    bonusReasons.push("good behavior at home");
  }
  if (entry.noMessAllDay) {
    bonusStars += 1;
    bonusReasons.push("no mess throughout the day");
  }
  if (entry.listenedToParents) {
    bonusStars += 1;
    bonusReasons.push("listened to mother and father");
  }

  if (bonusStars > 0) {
    reasons.push(`Bonus stars earned: ${bonusStars}.`);
    reasons.push(`Bonus sources: ${bonusReasons.join(", ")}.`);
  }
  ...
  const status = totalStars >= 12 ? "epic" : totalStars >= 8 ? "great" : totalStars >= 4 ? "good" : "tryAgain";
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#fff7ed_0%,#fdf2f8_25%,#eff6ff_70%,#eef2ff_100%)] p-3 text-slate-900 md:p-8">
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
          <ScoreCard label="Total stars" value={totalStars} subvalue="Homework stars plus daily habit bonus stars" glow />
          ...
          <div className="rounded-[32px] border border-white/60 bg-white/85 p-6 shadow-xl backdrop-blur">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <div className="text-sm text-slate-500">Track homework discipline plus extra stars for habits, self-play, and behavior through the day.</div>
              </div>
              <input
                type="date"
                value={selectedDate}
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <Toggle checked={form.selfPlayHour} onChange={() => updateField("selfPlayHour", !form.selfPlayHour)} label="1 hour of self-play" emoji="🧩" />
              <Toggle checked={form.constructivePlayHour} onChange={() => updateField("constructivePlayHour", !form.constructivePlayHour)} label="1 hour of constructive play" emoji="🛠️" />
              <Toggle checked={form.bathAndBrush} onChange={() => updateField("bathAndBrush", !form.bathAndBrush)} label="Took bath and brushed nicely" emoji="🛁" />
              <Toggle checked={form.goodBehaviorAtHome} onChange={() => updateField("goodBehaviorAtHome", !form.goodBehaviorAtHome)} label="Good behavior at home" emoji="🏠" />
              <Toggle checked={form.noMessAllDay} onChange={() => updateField("noMessAllDay", !form.noMessAllDay)} label="Did not create a mess all day" emoji="🧼" />
              <Toggle checked={form.listenedToParents} onChange={() => updateField("listenedToParents", !form.listenedToParents)} label="Said yes to mother and father" emoji="🙌" />
            </div>
            ...
            <div className="mt-6 rounded-[28px] bg-gradient-to-r from-fuchsia-100 via-amber-50 to-sky-100 p-4">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <div className="mt-1 text-3xl font-black text-slate-900">{previewResult.totalStars} / 15</div>
                  <div className="mt-1 text-xs text-slate-600">{previewResult.baseStars} homework stars + {previewResult.bonusStars} daily bonus stars</div>
                </div>
                <button
                  onClick={handleSave}
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              ...
              <div className="rounded-[24px] bg-slate-50 p-4 md:col-span-2">
                ...
                <div className="mt-2 text-base text-slate-700">
                  Every time total stars reach a multiple of 7, ₹250 gets added to the wallet. Bonus stars from self-play, constructive play, hygiene, behavior, cleanliness, and listening at home also count.
                </div>
              </div>
              <div className="rounded-[24px] bg-slate-50 p-4 md:col-span-2">
